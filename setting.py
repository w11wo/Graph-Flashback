...
class Setting:
    ...
    def parse_arguments(self, parser):
        ...
        # data management
        parser.add_argument(
            "--dataset", default="checkins-gowalla.txt", type=str, help="the dataset under ./data/<dataset.txt> to load"
        )
        parser.add_argument(
            "--friendship",
            default="gowalla_friend.txt",
            type=str,
            help="the friendship file under ../data/<edges.txt> to load",
        )
        # evaluation
        parser.add_argument(
            "--validate-epoch", default=5, type=int, help="run each validation after this amount of epochs"
        )
        parser.add_argument(
            "--report-user", default=-1, type=int, help="report every x user on evaluation (-1: ignore)"
        )

        # log
        parser.add_argument("--log_file", default="./results/log_gowalla", type=str, help="存储结果日志")
        parser.add_argument(
            "--trans_loc_file",
    ...
    def parse_gowalla(self, parser):
        ...
        parser.add_argument(
            "--batch-size", default=200, type=int, help="amount of users to process in one pass (batching)"  # 200
        )
    ...
    def parse_foursquare(self, parser):
        ...
        parser.add_argument(
            "--batch-size", default=512, type=int, help="amount of users to process in one pass (batching)"
        )  # 1024
        parser.add_argument("--lambda_t", default=0.1, type=float, help="decay factor for temporal data")
    ...
    def __str__(self):
        return (
            (
                ...
                if self.guess_foursquare
                else "parse with gowalla default settings"
            )
            + "\n"
