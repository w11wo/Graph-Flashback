...
def get_parser():
    ...
    parser.add_argument(
        "--pretrain_model",
        default="../data/pretrained-model/foursquare_scheme1/foursquare-transr-1641218589.ckpt",
        type=str,
...
def main():
    ...
    if args.model_type == "transh":
        ...
        if args.loc_graph:
            ...
            friend_graph = construct_transition_graph(
                args, graph_file, user_encoder, friend_preference, norm=norm_friend
            )
            interact_graph = construct_interact_graph(
                args, user_encoder, loc_encoder, interact_preference, norm=norm_interact
            )
        ...
        if args.loc_graph:
            ...
            friend_graph = construct_transition_graph(
                args, graph_file, user_encoder, friend_preference, proj=proj_friend
            )
            interact_graph = construct_interact_graph(
                args, user_encoder, loc_encoder, interact_preference, proj=proj_interact
            )
            friend_graph = friend_graph + interact_graph
            with open(graph_file, "wb") as f:
                pickle.dump(friend_graph, f, protocol=2)
